...
# Paths ----------------------------------------------------------------------------
    ...
    landcover_biophysical_table_path <- "envirospace/projects/GE21/IE/..."
...
# Parameters ------------------------------------------------------------------------
    ...
    description <- paste0("Capacité des écosystèmes à soutenir les pollinisateurs (modèle InVEST).", "\n",
                          "http://releases.naturalcapitalproject.org/invest-userguide/latest/en/croppollination.html")
    ...
    # 1.4) Load and prepare data --------------------------------------------------------
     ...
     # -----------------------------------------------------------------------------------
        ...
        info <- c(info, paste0("INPUTS : ", "\n",
                               ...
                               " * Raster LULC  : ", landcover_raster_path, "\n",
                               " * Table des guildes : ", guild_table_path, "\n"))
        info <- c(info, info_guild_table) 
        info <- c(info, paste0(" * Table biophysique : ", "\n"))
