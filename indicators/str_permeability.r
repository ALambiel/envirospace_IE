...
#                                                                                  #
#   This script allows you to map permeability by calculating the percentage of    #
#   the pixel's surrounding area occupied by permeable surfaces. Its calculation   #
#   is based on the "PERMEABLE" attribute, which has a value of 1 for permeable    # 
#   surfaces and 0 for impermeable ones.                                           # 
#                                                                                  #
...
#   assigns a permeability value to each pixel in the 5m resolution habitat map,   #
...
#   Focal Statistics (mean value over a circle of 200 m radius) of  the permea-    #
#   bility class raster.The final raster is then aggregated to 25m  resolution and #
...
# Paths ----------------------------------------------------------------------------
    ...
    #  ==>   The raster specified in 'study_area_path' will be used as a reference, meaning that the products will have the same resolution, extent and CRS as it. An aggregated 25m version of the final raster will then be produced. 
    ...
    # Attribute table linking all categories of the habitat map with a value of permeability (.csv, sep=",") 
...
# Parameters ------------------------------------------------------------------------
    ...
    indicator_name <- "PERMEABILITE"
    # Give a short descrition of the indicator
    description <- paste0("Perméabilité du territoire calculée comme le pourcentage de la zone environnante d'un pixel occupée par des surfaces perméables.", "\n",
                          "L'indice prend des valeurs comprises entre 0 (imperméable) et 1 (perméable). Pourcentage du territoire étudié couvert par des surfaces perméables.")
    ...
    # Name of the column in 'mn_attribute_table_path' corresponding to the permeability index value
    col_table_index <- "PERMEABLE"
    ...
    # Maximum ratio of NA present in a window for the permeability calculation to be carried out
    na_threshold <- 0.5
...
tryCatch({
    ...
    # 1.2) Packages and functions -------------------------------------------------------
        ...
        # mean_permeability(values, threshold) = calculates the mean of the vector of permeability "values", by first checking that the ratio of NA present in the "values" is not greater than a "threshold". This function is intended for use in the "focal()" function. 
        mean_permeability <- function(values, threshold){
    ...
    # 2.1) Assigning permeability values ------------------------------------------------
        write(paste(Sys.time(), "ASSIGNING PERMEABILITY VALUES"), tracking_file, append=TRUE)
        start_time <- Sys.time()
    
        # Reclassification of habitat map categories in permeability indices
    ...
    # 2.2) Rasterize the reclassified habitat map -----------------------------------------
        ...
        # Extract only informations about permeability and save it in an other vector layers 
        permeability_vector <- habitat_map_vector["RECLASS"]
        # Rasterize it with the study area raster as a canva (same resolution, extent and CRS)
        permeability_raster <- rasterize(permeability_vector, study_area_raster, field = "RECLASS")
    
        # Extend the rasterised permeability map by Euclidean allocation to avoid edge effect
        extended_permeability_raster <- extend_by_euclidean_allocation(permeability_raster, buffer_size)
        
        # Save temporary layers in scratchfold
        writeRaster(permeability_raster, file.path(scratch_folder, "mn_permeabilite.tif"), overwrite=TRUE)
        writeRaster(extended_permeability_raster, file.path(scratch_folder, paste0("mn_permeabilite_buff", buffer_size, "m.tif")), overwrite=TRUE)

        # Free memory
        rm(permeability_raster)
        gc()
    ...
    # 2.3) Compute focal statistics ---------------------------------------------------------
        ...
        circular_window <- focalMat(extended_permeability_raster, circular_window_size, "circle")
        ...
        # Get suggested chunk sizes to crop the permeability raster in sub-rasters
        ...
        # Apply the 'mean_permeability()' funciton through the 'focal()' function, for each block in parallel
        results <- foreach(block = blocks_list, .combine = 'c', .packages = c('terra', 'raster')) %dopar% {
            focal(block, w = circular_window, fun = function(x){mean_permeability(x, na_threshold)}, na.rm=FALSE)
            }
        # Close the cluster
        stopImplicitCluster()
        ...
        # Save it
        writeRaster(focal_raster, file.path(scratch_folder, "mn_permeabilite_focal_brute.tif"), overwrite=TRUE)
    ...
    # -----------------------------------------------------------------------------------
        ...
        info <- c(info, paste0("STATISTIQUES ZONALES ", "\n",
                               " * Fonction appliquée : ", "\n"))
        info <- c(info, deparse(mean_permeability))
    ...
    # 2.4) Prepare final result ---------------------------------------------------------
        ...
        # Save outputs
        writeRaster(final_raster, file.path(output_folder, paste0("mn_permeabilite_focal_", res(final_raster)[1], "m.tif")), overwrite = TRUE)
        if (!is.null(do_aggregate)) {
          writeRaster(aggregated_raster, file.path(output_folder, paste0("mn_permeabilite_focal_", unique(res(final_raster)*do_aggregate), "m.tif")), overwrite = TRUE)
        }
