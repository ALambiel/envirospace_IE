...
# Parameters ------------------------------------------------------------------------
    ...
    indicator_name <- "PERMEABILITY"
    # Give a short descrition of the indicator
    description <- paste0("Land permeability calculated as the percentage of a pixel's surrounding area occupied by permeable surfaces.", "\n",
                          "The index takes values between 0 (impermeable) and 1 (permeable). Percentage of the study area covered by permeable surfaces.")
...
tryCatch({
    ...
    # 2.2) Rasterize the reclassified habitat map -----------------------------------------
        ...
        writeRaster(permeability_raster, file.path(scratch_folder, "mn_permeability.tif"), overwrite=TRUE)
        writeRaster(extended_permeability_raster, file.path(scratch_folder, paste0("mn_permeability_buff", buffer_size, "m.tif")), overwrite=TRUE)
    ...
    # 2.4) Prepare final result ---------------------------------------------------------
        ...
        # Save outputs
        writeRaster(final_raster, file.path(output_folder, paste0("mn_permeability_focal_", res(final_raster)[1], "m.tif")), overwrite = TRUE)
        if (!is.null(do_aggregate)) {
          writeRaster(aggregated_raster, file.path(output_folder, paste0("mn_permeability_focal_", unique(res(final_raster)*do_aggregate), "m.tif")), overwrite = TRUE)
        }
