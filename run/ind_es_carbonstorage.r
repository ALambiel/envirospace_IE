####################################################################################
############## Carbon Storage and Sequestration modelling with InVEST ##############
####################################################################################
#                                                                                  #
#                        PLEASE READ BEFORE RUNNING                                #
#                                                                                  #
#   This script allows you to model carbon storage in an area using the InVEST     # 
#   Carbon Storage and Sequestration model, which aggregates the amount of carbon  #
#   stored in the 4 carbon pools (aboveground biomass, belowground biomass, soil,  #
#   and dead organic matter) according to land use maps and classifications you    #
#   provide.                                                                       #
#                                                                                  #
...
# Paths ----------------------------------------------------------------------------
    ...
    #       https://invest.readthedocs.io/en/latest/models.html#carbon-storage-and-sequestration
...
# Parameters ------------------------------------------------------------------------
    ...
    # See here: http://releases.naturalcapitalproject.org/invest-userguide/latest/en/carbonstorage.html#interpreting-results
    pattern_invest_output <- "tot_c_cur"
...
tryCatch({
    ...
    # 1.4) Load and prepare data --------------------------------------------------------
        ...
        carbon_pools_table <- read.csv(carbon_pools_path, sep=",", header=TRUE)
        lulc_list <- unique(carbon_pools_table$LULC_name)
        # Save information from carbon pools table
        info_carbon_pools_table <- capture.output(print(carbon_pools_table))
    
        # Read landcover raster and make sure to align it with study area raster 
        lulc_cur_aligned_path <- align_spatial_object(lulc_cur_path, study_area_raster, scratch_folder)
    ...
    # 2.1) Prepare the Python script ----------------------------------------------------
        ...
        py_script_path <- paste(scratch_folder, "investC.py", sep="/")
        file(py_script_path, open="a")
        # Prepare all the lines
        py_script <- paste0("import natcap.invest.carbon", "\n", # Import the carbon storage and sequestration InVEST model from the natcap module
                            "import csv", "\n", # Import csv module to read table
                            invest_arguments, "\n", # Read the arguments 
                            "natcap.invest.carbon.execute(args)") # Execute the model 
    ...
    # 2.3) Prepare final result ---------------------------------------------------------
        ...
        # Save the result  
        writeRaster(final_raster, file.path(output_folder, "carbon_storage_total_pred.tif"), overwrite=TRUE)
