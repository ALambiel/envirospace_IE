...
#                                                                                  #
#   This script allows you to map the naturalness of an area, i.e. the extent to   # 
#   which an environment or space has been subject to human influence that could   #
#   potentially impact its ability to fulfil its natural functions.                #
#   The script is based on a naturalness index with values ranging from 1 (the     #
#   most anthropised environments) to 5 for the most natural environments.         #
#                                                                                  #
...
#   assigns a naturalness value to each pixel in the 5m resolution habitat map,    #
#   based on the attribute table provided. The map of habitat is then rasterised   #
#   on the model of the raster of the study area, and extended outside  this area  #
#   by Euclidean allocation. The final final naturalness raster is calculated by   #
#   Focal Statistics (mean value over a circle of 200 m radius) of  the natural-   #
#   ness class raster.The final raster is then aggregated to 25m  resolution and   #
#   clipped to the study area raster.                                              #
#                                                                                  #
...
# Paths ----------------------------------------------------------------------------
    ...
    # Attribute table linking all categories of the habitat map with a value of naturalness (.csv, sep=",") 
...
# Parameters ------------------------------------------------------------------------
    ...
    indicator_name <- "NATURALNESS"
    # Give a short descrition of the indicator
    description <- paste0("A measure of the human influence on an environment or area, which can potentially impact on its ability to fulfil its natural functions.", "\n", 
                          "The naturalness index has values ranging from 1 (most anthropogenic environments) to 5 (most natural environments).", "\n",
                          "This classification is the result of adapting the method based on the urban character index of O'Neill et al. (1988) to the scale of Geneva's regions.")
    ...
    # Maximum ratio of NA present in a window for the naturalness calculation to be carried out
    na_threshold <- 0.5
...
tryCatch({
    ...
    # 1.2) Packages and functions -------------------------------------------------------
        ...
        # mean_naturalness(values, threshold) = calculates the mean of the vector of naturalness "values", by first checking that the ratio of NA present in the "values" is not greater than a "threshold". This function is intended for use in the "focal()" function. 
        mean_naturalness <- function(values, threshold){
    ...
    # -----------------------------------------------------------------------------------
    # 2) Compute naturalness -------------------------------------------------------------
    # 2.1) Assigning naturalness values --------------------------------------------------
        write(paste(Sys.time(), "ASSIGNING NATURALNESS VALUES"), tracking_file, append=TRUE)
        start_time <- Sys.time()
    
        # Reclassification of habitat map categories in naturalness values
    ...
    # -----------------------------------------------------------------------------------
        ...
        info <- c(info, paste0("NATURALNESS VALUES ASSIGNATION", "\n",
                               " * Matching table: ", "\n",
    ...
    # 2.2) Rasterize the reclassified habitat map ---------------------------------------
        ...
        # Extract only informations about naturalness and save it in an other vector layers 
        naturalness_vector <- habitat_map_vector["RECLASS"]
        # Rasterize it with the study area raster as a canva (same resolution, extent and CRS)
        naturalness_raster <- rasterize(naturalness_vector, study_area_raster, field = "RECLASS")
    
        # Extend the rasterized naturalness map by Euclidean allocation to avoid edge effect
        extended_naturalness_raster <- extend_by_euclidean_allocation(naturalness_raster, buffer_size)

        # Save temporary layers in scratch folded
        writeRaster(naturalness_raster, file.path(scratch_folder, "mn_naturalness.tif"), overwrite=TRUE)
        writeRaster(extended_naturalness_raster, file.path(scratch_folder, paste0("mn_naturalness_buff", buffer_size, "m.tif")), overwrite=TRUE)

        # Free memory 
        rm(naturalness_raster)
        gc()
    ...
    # 2.3) Compute focal statistics ---------------------------------------------------------
        write(paste(Sys.time(), "COMPUTE FOCAL STATISTICS"), tracking_file, append=TRUE)
        start_time <- Sys.time()
    
        # Create a mooving circular window
        circular_window <- focalMat(extended_naturalness_raster, circular_window_size, "circle")
        # Attribue the same weight (1) for all cells in the window, those outside are not considered (NA)
        circular_window <- ifelse(circular_window == 0, NA, 1)

        # Get suggested chunk sizes to crop the naturalness raster in sub-rasters
        ...
        # Apply the 'mean_naturalness()' funciton through the 'focal()' function, for each block in parallel
        results <- foreach(block = blocks_list, .combine = 'c', .packages = c('terra', 'raster')) %dopar% {
            focal(block, w = circular_window, fun = function(x){mean_naturalness(x, na_threshold)}, na.rm=FALSE)
            }
        # Close the cluster
        stopImplicitCluster()
        ...
        focal_raster <- rast(focal_raster) # come back to a SpatRaster 

        # Save it
        writeRaster(focal_raster, file.path(scratch_folder, "mn_naturalness_focal_raw.tif"), overwrite=TRUE)
    ...
    # -----------------------------------------------------------------------------------
        ...
        info <- c(info, paste0("ZONAL STATISTICS", "\n",
                               " * Applied function: ", "\n"))
        info <- c(info, deparse(mean_naturalness))
    ...
    # 2.4) Prepare final result ---------------------------------------------------------
        ...
        # Save outputs
        writeRaster(final_raster, file.path(output_folder, paste0("mn_naturalness_focal_", res(final_raster)[1], "m.tif")), overwrite = TRUE)
        if (!is.null(do_aggregate)) {
          writeRaster(aggregated_raster, file.path(output_folder, paste0("mn_naturalness_focal_", unique(res(final_raster)*do_aggregate), "m.tif")), overwrite = TRUE)
        }
    ...
    # -----------------------------------------------------------------------------------
        ...
        info <- c(info, paste0("OUTPUTS", "\n",
                               ...
                               " * Aggregation function: ", if (!is.null(do_aggregate)) aggregation_function, "\n",
...
if (track_to_trash == "YES" & file.size(error_file) == 0) { # if the user has chosen "YES" to allow deletion AND and that no errors have occurred
